...
**Starting point:** `18d229df90dfd8234b5c2e1f3c7b887c82afdb75`  
**Accepted state:** Stage 1, Stage 1A, Stage 1B and Stage 2 are complete. Stage 3 is next.  
...
- required book or source examination means actually examining the sources, not using model memory or summaries.
- required execution means executing it, not merely writing a script or describing the expected result.
- required comparison or benchmark means recording the actual comparison or benchmark evidence.
- required playable proof means an actual playable proof at the fidelity the stage requires, not only a design document.

Never invent sources, playtest evidence, telemetry, benchmark results, tool executions, installation results, player observations or test passes. Synthetic fixtures must remain explicitly synthetic.
...
User-provided books must follow the bootstrap's explicit replacement/demotion rules. Never silently remove or substitute them.
